...
int main (int argc, char *argv[])
{
	...
	bool neuralTree=false; bool testing=false;bool testingAcc=false; bool testingTree=false; bool famNames=false; bool printTreeClusts=false;
	bool ma_off=false;
				...
				if(pairwiseOnly){
					...
					if(!testingAcc){
						if(tree_loocv && testingTree){
							...
							if(!silent){printf("The Calinski & Harabasz statistic suggests %.0lf clusters in the input motifs\n", T->GetNodesMinCH());}
							if(printTreeClusts){
