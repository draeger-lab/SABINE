...
public class FormatConverter {
	...
	public String[] parseInternalFormat(String infile) {
		...
		// parse class, if available	
			...
			if(line.startsWith("CL  ")) {
				...
				class_id = "class" + getTransfacClass(line.substring(4)).substring(0,1);
		...
		// parse sequence(s)
			...
			if(line.startsWith("S1  ")) {
				sequence1 = line.substring(4,line.length()).trim();
			...
			if(line != null && line.startsWith("S2  ")) {
				sequence2 = line.substring(4,line.length()).trim();
	...
	//	Method 2: check whether class is obtainable by its class_id ...	
		...
		if(class_id.substring(0,6).matches(CLASS_ID_FORMAT)) {
