...
public class DatabaseParser {
	...
	public void writeToFile(String protID_outfile,
		...
		try {
			...
			if (geneID_outfile != null) bw_gene = new BufferedWriter(new FileWriter(new File(geneID_outfile)));
			...
			for (int i=0; i<hmm_IDs.size(); i++) {
				...
				if (bw_gene != null && !gene_IDs.isEmpty()) bw_gene.write(gene_IDs.get(i) + "\n");
				if (bw_gene != null && !transfac_IDs.isEmpty()) bw_gene.write(transfac_IDs.get(i) + "\n");
			}
			bw_prot.flush();
			bw_prot.close();
			bw_seq.flush();
	...
	public void readFromFile(String protID_infile, 
		...
		try {
			...
			if (geneID_infile != null) br_gene = new BufferedReader(new FileReader(new File(geneID_infile)));
			...
			if (br_gene != null && !geneID_infile.contains("transfac")) {
				while ((line = br_prot.readLine()) != null) prot_IDs.add(line);
				while ((line = br_gene.readLine()) != null) gene_IDs.add(line);
			}
			else if (br_gene != null && geneID_infile.contains("transfac")) {
				while ((line = br_prot.readLine()) != null) uniprot_IDs.add(new String[] {line});
				while ((line = br_gene.readLine()) != null) transfac_IDs.add(line);
			}
			else {
				while ((line = br_prot.readLine()) != null) uniprot_IDs.add(new String[] {line});
			}
			while ((line = br_seq.readLine()) != null)  sequences.add(line);
			
			br_prot.close();
			br_seq.close();
			if (br_gene != null) br_gene.close();
		}
	...
	public void extractTestSet(String infile, String outfile, boolean use_transfac_AC) {
		...
		try {
			...
			for (int i=0; i<print_perm.length; i++) print_perm[i] = testset_idx.get(i);
	...
	public void runDatabaseParser(String organism, String database, String base_dir) {
		...
		if      (organism.equals("human"))   species_name = "Homo sapiens";
		else if (organism.equals("mouse"))   species_name = "Mus musculus";
		else if (organism.equals("rat"))     species_name = "Rattus norvegicus";
		else                                 usage();
		...
		if (database_dom != null) System.out.println("             " + database_dom);
		System.out.println();
		System.out.println("  Base directory: " + base_dir);
		System.out.println("\n");
		...
		if (!gene_IDs.isEmpty()) {
			...
			for (int i=0; i<uniprot_IDs.size(); i++) {
				if (!uniprot_IDs.get(i)[0].equals("NA")) succ_parsed_IDs++;
			}
